...
export const Header = () => {
    ...
    return (
        <>
            <header className="fixed top-0 left-0 right-0 z-[100] bg-white/90 backdrop-blur-md border-b border-zinc-200 h-16 shadow-sm">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                        <Logo />
                        <button
                            className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-zinc-100 hover:bg-zinc-200 border border-zinc-200 rounded-full transition-all group"
                            onClick={() => {
                        ...
                        >
                            ...
                            <span className="text-[11px] font-bold text-zinc-600 group-hover:text-zinc-900 transition-colors">
                                {currentNode}
                            </span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        {user && (
                            <div className="hidden md:flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-1.5 px-3 py-1.5 bg-[#FF6200]/10 border border-[#FF6200]/20 rounded-full cursor-help" title="MarketCoins balance">
                                    ...
                                    <span className="text-xs font-black text-zinc-900">{(user.coins_balance || 0).toLocaleString()}</span>
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <button className="flex items-center gap-2 pl-1.5 pr-3 py-1.5 rounded-full border border-zinc-200 bg-white hover:bg-zinc-50 transition-all shadow-sm group">
                                            ...
                                            <span className="text-xs font-bold text-zinc-700 group-hover:text-zinc-900 transition-colors max-w-[80px] truncate">
                                    ...
                                    <DropdownMenuPortal>
                                        <DropdownMenuContent
                                            align="end"
                                            sideOffset={12}
                                            className="w-64 bg-white border border-zinc-200 p-1.5 text-zinc-900 z-[999] shadow-2xl rounded-2xl"
                                        >
                                            {/* Account info */}
                                            <div className="px-3 py-3 mb-1 border-b border-zinc-100 bg-zinc-50/50 rounded-lg">
                                                <p className="text-xs font-black text-zinc-900 truncate">{user.displayName || 'MarketBridge User'}</p>
                                            ...
                                            <DropdownMenuItem
                                                ...
                                                className="rounded-xl cursor-pointer focus:bg-red-50 text-red-600 my-0.5 hover:text-red-700"
                                            >
                        ...
                        {/* Mobile Hamburger */}
                        <button
                            className="md:hidden h-9 w-9 flex items-center justify-center rounded-full border border-zinc-200 bg-zinc-50 hover:bg-zinc-100 transition-all"
                            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                            aria-label="Toggle menu"
                        >
                            {mobileMenuOpen ? <X className="h-4 w-4 text-zinc-900" /> : <Menu className="h-4 w-4 text-zinc-900" />}
                        </button>
                    </div>
                </div>
            </header>
            ...
            {mobileMenuOpen && (
                <div className="fixed top-16 left-0 right-0 z-[99] bg-white border-b border-zinc-200 px-6 py-6 flex flex-col gap-4 md:hidden shadow-xl">
                    {navLinks.map((link) => (
                        <Link
                            key={link.href}
                            href={link.href}
                            onClick={() => setMobileMenuOpen(false)}
                            className="text-zinc-600 hover:text-zinc-900 font-bold text-lg transition-colors p-2"
                        >
                            {link.label}
                        </Link>
                    ))}
                    <div className="border-t border-zinc-100 pt-4 flex flex-col gap-3 mt-2">
                        {user ? (
                            <>
                                <Link href="/settings" onClick={() => setMobileMenuOpen(false)} className="text-zinc-600 font-bold p-2">My Account</Link>
                            ...
                            <>
                                ...
                                <Link href="/login" onClick={() => setMobileMenuOpen(false)} className="w-full py-4 rounded-2xl border border-zinc-200 bg-zinc-50 text-zinc-900 font-bold text-center text-sm">
                                    Log In
                                </Link>
                            </>
                        )}
