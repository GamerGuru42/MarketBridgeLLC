...
foreach ($file in $files) {
    try {
        ...
        foreach ($old in $mappings.Keys) {
            # Regex for case-insensitive replacement with word boundaries for Signal/Protocol/Terminal
        ...
        Write-Warning "Failed to refine $($file.FullName): $($_.Exception.Message)"
    }
}
